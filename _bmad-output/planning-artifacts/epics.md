...
NFR5 (Latency, benchmarked): Prompt acknowledgment targets ≤3 seconds and must remain inside AppSync's response boundary. Full generation is asynchronous; current durable live evidence is approximately 34–36 seconds and the client follows the exact Session, retaining its recovery id if status is still indeterminate at 300 seconds.
...
**Then** the system atomically reserves one DailyUsage unit and the estimated MonthlySpend cost up front (AD-6), draws Card(s), calls Tavily and retains up to 3 valid Current Events, then calls Claude Opus to produce an essay-form Guide covering FR8's five-part structure
