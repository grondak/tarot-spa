...
## Deferred from: code review of story-4-3 (2026-09-25)

- **`AdminConfigEditor` never re-syncs its local state if `dailyLimit`/`monthlyBudget` props change after mount** (no effect/key tying identity to `metrics.config`). Not reachable via any current call site — the only thing that changes `metrics.config` today is this same component's own save. Fixing it well requires deciding how to handle a future background refresh clobbering an admin's in-progress unsaved edit, which is a design call, not a rote patch.
- **This diff bundles the unrelated Story 4-5 backlog file and its `sprint-status.yaml` action item into what should be a scoped feature commit** (`53933cc`). Already pushed; not worth a history rewrite on a solo-owned project.
- **`COST_ESTIMATE_USD` single-sourcing between the worker and the schema is enforced only by one equality test, not structurally.** No unambiguous fix without disproportionate tooling (e.g. a magic-number lint rule); the existing invariant test is adequate coverage today.
...
- **(Round 2 re-review) `amplify/data/resource.test.ts`'s field-completeness regex would silently miss a hypothetical future Config field declared in a two-line style** (name+`a` on one line, type-chain starting the next — the style this repo's own top-level model declarations use, though no field currently does). No current field uses this style, and the test file's own comment already discloses it's an intentionally lightweight, text-based contract check rather than a full parser.
