...
- Remove the real invite key from repository artifacts and rotate or invalidate it if it remains redeemable. The same key was already committed in Story 1.2, so fixing the Story 1.4 copy alone would not remove the exposure.
  - **2026-07-16 (Epic 1 close-out):** both live-minted key codes (the UI mint from 1.2 and the earlier direct-Lambda test mint) redacted from all current story artifacts. The codes remain in git history, so live invalidation is the actual control — **pending**: verify both keys' status in the sandbox InviteKey table and revoke/delete any that are still `unredeemed` (blocked on AWS SSO re-auth at the time of the sweep).
