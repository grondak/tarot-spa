...
- [ ] **Task 8: Infrastructure clarity and live verification** (AC: all)
  ...
  - [ ] As Tony/admin, open Admin Dashboard, change values within range, save, and verify the existing Config item changed (same `id: global`, no second row); dashboard fields and spend denominator show returned values. **Left unchecked deliberately** — Tony performed this exact walkthrough and the Config item *did* change correctly server-side (`dailyLimit` → `7`, confirmed on revisit), but the "dashboard fields... show returned values" half was never actually observed: the UI kicked him out mid-save before any success status rendered, per the pre-existing bug filed as Story 4-5. This bullet stays unticked as an honest record that the full walkthrough-as-written wasn't cleanly demonstrated, rather than checking it off on partial evidence.
...
*Three-layer parallel review (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against `f95a334..HEAD`. 0 decision-needed, 10 patch, 4 defer, 7 dismissed after reading the actual code at each location rather than rating from diff hunks alone.*

**Patch** (all applied 2026-09-25 — see Debug Log for verification)

- [x] [Review][Patch] `AdminConfigEditor.jsx` hand-copies validation bounds/messages instead of importing them from `amplify/config.ts` — the Dev Notes' single-source-of-truth claim isn't structurally enforced [src/components/AdminConfigEditor.jsx:6-13]
- [x] [Review][Patch] Blank input is rejected only by coincidence (`Number('')` is `0`, which just happens to fall below both positive minimums), not by an explicit check [src/components/AdminConfigEditor.jsx:17-32]
- [x] [Review][Patch] When both fields are invalid on submit, only the daily-limit error is ever shown (`??` short-circuit silently drops the second) [src/components/AdminConfigEditor.jsx:57]
- [x] [Review][Patch] No `<form>` wrapper — pressing Enter in either input does not submit; only a direct click on Save works [src/components/AdminConfigEditor.jsx:88-131]
- [x] [Review][Patch] Component-level test never submits `dailyLimit = 100` (the true max) and asserts acceptance — only rejection at 101 and the *minimum* boundary are exercised at this layer [src/components/AdminConfigEditor.test.jsx]
- [x] [Review][Patch] The new `AdminDashboard` integration test only exercises the happy path through the real `adminConfig.js`/`generateClient` composition — the AppSync `errors`-array rejection path is never proven end-to-end through the actual composed dashboard [src/components/AdminDashboard.test.jsx]
- [x] [Review][Patch] `amplify/backend.ts`'s `AWS_SAFETY_CEILING_USD` and `amplify/config.ts`'s `MAX_MONTHLY_BUDGET_USD` are two independent literal `30`s with no code-level link enforcing "Config ceiling ≤ AWS safety ceiling" [amplify/backend.ts]
- [x] [Review][Patch] `amplify/data/resource.test.ts`'s schema-contract test is mechanically brittle: exact-whitespace string matching, a hardcoded 6-space-indent regex, a field-enumeration regex restricted to `integer`/`float` (silently missing a future non-numeric field), and an unbounded fallback slice-to-EOF if Config were ever the last model in the schema [amplify/data/resource.test.ts]
- [x] [Review][Patch] Task 8's live-dashboard-walkthrough bullet is left unchecked while Task 9 (gated on Task 8) is fully checked — an unannotated internal inconsistency; the bullet needs an honest note explaining what was actually observed, not a silent contradiction [this file, Task 8/Task 9]
- [x] [Review][Patch] Story 4-5's `sprint-status.yaml` action-item entry carries no severity/priority signal despite documenting a live AC10 violation in production today [_bmad-output/implementation-artifacts/sprint-status.yaml]

**Defer**

- [x] [Review][Defer] `AdminConfigEditor` never re-syncs its local state if `dailyLimit`/`monthlyBudget` props change after mount (no effect/key tying identity to `metrics.config`) [src/components/AdminConfigEditor.jsx:40-41] — not reachable via any current call site (the only thing that changes `metrics.config` today is this same component's own save); fixing it well requires deciding how to handle a future background refresh clobbering an admin's in-progress unsaved edit, which is a design call, not a rote patch
- [x] [Review][Defer] This diff bundles the unrelated Story 4-5 backlog file and its `sprint-status.yaml` action item into what should be a scoped feature commit [commit `53933cc`] — already pushed; not worth a history rewrite on a solo-owned project
- [x] [Review][Defer] `COST_ESTIMATE_USD` single-sourcing between the worker and the schema is enforced only by one equality test, not structurally — no unambiguous fix without disproportionate tooling (e.g. a magic-number lint rule); the existing invariant test is adequate coverage today
- [x] [Review][Defer] Live-verification evidence for the authorization-boundary checks (wrong-id, non-admin, out-of-range) is recorded as narrated prose in the Dev Agent Record rather than attached raw response artifacts — matches this project's established convention for prior stories' live evidence, and the exact error type/message text were preserved verbatim

**Dismissed (7)** — unmount-during-pending-save losing `onSaved` (self-heals: `AdminDashboard` fully remounts and refetches on every dashboard open); `adminConfig.js`'s `!data` check missing partial-field detection (unreachable — both Config fields are now `.required()` at the schema level, so AppSync cannot return a successful update missing either); `AdminDashboard.jsx` accessing `metrics.config` without optional chaining (unreachable post-deploy — `admin-metrics` unconditionally includes `config` in every response now); no concurrent-multi-admin-write conflict handling (explicitly out of scope per this story's own "multi-admin conflict/version UI" exclusion); `updateAdminConfig` collable with only one field defined (unreachable — the only real caller always supplies both, per the frozen "save both together" contract); `admin-metrics` surfacing an invalid/legacy Config value verbatim (not possible — it reads through the same shared, validated `readConfig` this story hardened in Task 1); untested many-decimal-digit `monthlyBudget` display (explicitly permitted by this story's own Dev Notes — "any finite in-range float is valid... do not invent a scale validator").

### Review Findings, Round 2 (2026-09-25) — re-review after Round 1's patches

*Same three-layer parallel review, re-run against `f95a334..working-tree` (baseline through Round 1's applied patches). Verified each Round 1 patch claim against actual code rather than trusting the story's own narrative — found two real problems with the fixes themselves (see Decision and Patch 6 below). 1 decision-needed (resolved), 9 patch, 2 defer, 7 dismissed.*

**Decision — resolved**

- [x] [Review][Decision] Round 1's fix for hand-duplicated frontend bounds/messages (`AdminConfigEditor.jsx` now imports directly from `amplify/config.ts`) technically deviates from the frozen Dev Notes contract, which specified duplicated-but-test-guarded frontend constants and only listed backend files as importers of `amplify/config.ts`. Also makes the "mirrors bounds" tests tautological (both sides resolve to the identical import) and wires backend TS source into the client bundle with no guard against a future backend-only change breaking the frontend build. **Resolved by Tony: keep the direct import (it does eliminate drift structurally), add a bundlability guard instead of reverting.**

**Patch** (all applied 2026-09-25)

- [x] [Review][Patch] Per the resolved decision above: add a test asserting `amplify/config.ts` has no imports/side effects, so a future backend-motivated change can't silently break the frontend bundle [amplify/config.test.ts]
- [x] [Review][Patch] `readConfig`'s error message says "orientation config missing — run scripts/seed-config.mjs" even when the item is present-but-invalid, not missing — misleading, since re-seeding (conditional on absence) won't fix an invalid-but-present row; Task 1 itself flagged distinguishing these as a nice-to-have [amplify/functions/usage-counter/reservation.ts]
- [x] [Review][Patch] Round 1's dual-field-invalid combined-message fix has zero test coverage [src/components/AdminConfigEditor.test.jsx]
- [x] [Review][Patch] Round 1's Enter-to-submit `<form>` wrapper has zero test coverage — every test still only clicks the Save button [src/components/AdminConfigEditor.test.jsx]
- [x] [Review][Patch] `amplify/backend.ts`'s new ceiling-vs-max invariant check has zero test coverage, and this file has no existing test pattern to extend — extracted the comparison into a pure, testable helper in `amplify/config.ts` instead [amplify/backend.ts, amplify/config.ts]
- [x] [Review][Patch] `resource.test.ts`'s `extractModelBlock` header-matching is still indentation-coupled (now an exact 2-space literal instead of 6) — made whitespace-tolerant [amplify/data/resource.test.ts]
- [x] [Review][Patch] Round 1's "bounded EOF fallback" branch is dead code against the real file — `checkInviteKey` immediately follows `Config` and already satisfies the next-header regex, so the fallback never executes, and the comment claiming "Config is the last model" is factually wrong. Refactored `extractModelBlock` to take a source string as a parameter and added a dedicated synthetic-input test that actually exercises the fallback bound; corrected the comment [amplify/data/resource.test.ts]
- [x] [Review][Patch] The new `AdminDashboard` error-path integration test only asserts `dailyLimit` retention on failure, not `monthlyBudget`/the displayed budget line [src/components/AdminDashboard.test.jsx]
- [x] [Review][Patch] `noValidate` removed native browser validation-state association — added `aria-invalid`/`aria-describedby` linking each input to the rendered error so screen readers don't lose that signal [src/components/AdminConfigEditor.jsx]

**Defer**

- [x] [Review][Defer] `resource.test.ts`'s field-completeness regex would silently miss a hypothetical future Config field declared in a two-line style (name+`a` on one line, type-chain starting the next) — no current field uses this style, and this test file's own comment already discloses it's an intentionally lightweight, text-based contract check, not a full parser
- [x] [Review][Defer] Live-verification evidence for the authorization-boundary checks remains recorded as prose with no attached raw artifact — same carry-forward item as Round 1; a re-review has no way to independently confirm it beyond the narrative

**Dismissed (7)** — `readConfig` rejecting an invalid-but-present stored Config item as a fail-closed error (this is AC8's explicitly frozen, intended behavior, not a defect); `metrics.config` accessed without optional chaining (reconfirmed unreachable — if Config were ever invalid, `admin-metrics` itself throws before returning any response at all, so `AdminDashboard`'s error state handles it upstream); `Number()` accepting scientific/hex notation (unreachable through the real `<input type="number">` UI, which restricts typable characters; a direct bypass hits server-side schema validation instead); `AdminConfigEditor`/`AdminDashboard` trusting a malformed `saved` payload from `updateAdminConfigFn` (unreachable in production — the real utility can't return a malformed object given the schema's required fields); initial `dailyLimit`/`monthlyBudget` props already out of bounds at mount (unreachable — `readConfig`'s fail-closed guard guarantees any value `admin-metrics` successfully returns is already valid); `adminConfig.js` surfacing only the first of multiple simultaneous AppSync validation errors (unreachable via the only real caller, whose own client-side validation gate prevents ever submitting two simultaneously-invalid values over the network).
...
- 2026-09-25 code review (`bmad-code-review`, three-layer parallel: Blind Hunter, Edge Case Hunter, Acceptance Auditor against `f95a334..HEAD`): all 10 `patch` findings applied — imported real bounds/messages into `AdminConfigEditor.jsx` instead of hand-copying them; added explicit blank-input rejection; combined both validation messages when both fields are invalid; wrapped inputs in a `<form noValidate>` so Enter submits (discovered and fixed a regression this introduced: native HTML5 min/max constraint validation was silently blocking submission of out-of-range values before our own JS validation could run — fixed with `noValidate`); added a `dailyLimit = 100` maximum-boundary acceptance test; added an `AdminDashboard` integration test proving the AppSync `errors`-array rejection path through the real composed dashboard; linked `amplify/backend.ts`'s `AWS_SAFETY_CEILING_USD` to `amplify/config.ts`'s `MAX_MONTHLY_BUDGET_USD` with a synth-time assertion instead of two independent literal `30`s; hardened `resource.test.ts`'s schema-contract test (indentation-independent field regex, any field type not just integer/float, bounded EOF fallback); annotated Task 8's still-unchecked live-walkthrough bullet with an honest explanation; added a priority note to Story 4-5's `sprint-status.yaml` action item. Full suite re-verified after all patches: `npm test -- --run` 353/353 (up from 351 — the two new tests), lint/typecheck/build clean. Also updated two stale Epic-3 action items (`dailyLimit`/`monthlyBudget` validation; Config/AWS-Budget reconciliation) from `open` to `done` — both were explicitly the critical-path prep items this story was created to resolve, and both are now verified done. 4 findings deferred to `deferred-work.md`; 7 dismissed after reading the actual code (several Edge Case Hunter findings were already structurally prevented by this story's own Task 1/2 changes).
- 2026-09-25 re-review (round 2, same three layers, re-run against `f95a334..working-tree` including round 1's patches): caught that round 1's fix for hand-duplicated frontend constants technically deviated from the frozen contract without being flagged (Scope Decision #7) — Tony resolved this by keeping the direct import and adding a bundlability guard rather than reverting. Also caught a factual error in round 1's own `resource.test.ts` fix: the comment claiming "Config is the last model" was wrong (`checkInviteKey` immediately follows it), making the added EOF-fallback branch dead/unexercised code — refactored `extractModelBlock` to accept a source string as a parameter and added a synthetic-input test that genuinely exercises that branch. Applied all 9 round-2 patches: bundlability guard test; distinguished `readConfig`'s "missing" vs "present but invalid" Config error messages (updated three dependent test files' assertions accordingly); added tests for the dual-error message and Enter-to-submit paths; extracted the `backend.ts` ceiling invariant into a testable pure function in `amplify/config.ts`; made `resource.test.ts`'s header matching whitespace-tolerant; extended the `AdminDashboard` error-path test to cover `monthlyBudget`/the budget line, not just `dailyLimit`; added `aria-invalid`/`aria-describedby` to restore the accessibility signal `noValidate` removed. Full suite re-verified: `npm test -- --run` 362/362 (up from 353), lint/typecheck/build clean. 2 findings deferred; 7 dismissed (several reconfirming round-1 dismissals, now on stronger grounds — e.g. `metrics.config` optional-chaining is doubly confirmed unreachable since `readConfig`'s fail-closed guard means `admin-metrics` throws before ever returning a response missing `config`).
- 2026-09-25: Committed and pushed the code-review fixes. `git status --short` (post-commit): empty. `git log -1 --oneline --decorate`: `2568d40 (HEAD -> main) fix: address story 4.3 code review findings (two rounds)`. Pushed to `origin/main`: `fef5928..2568d40  main -> main`.
...
- 2026-09-25: Ran `bmad-code-review` (three parallel layers against `f95a334..HEAD`, then triage). 0 decision-needed, 10 patch, 4 defer, 7 dismissed. Applied all 10 patches (see Debug Log for the full list, including a self-inflicted regression caught and fixed mid-patch: wrapping inputs in a `<form>` for Enter-to-submit initially broke out-of-range validation because native HTML5 constraint validation intercepted submission before our own JS validation ran; fixed with `noValidate`). Re-verified 353/353 tests, lint/typecheck/build clean. Closed two stale Epic-3 action items this story was created to resolve. Moved story to `done`.
