...
baseline_commit: (dev agent: record the full SHA of HEAD as your first action at dev start — do not inherit a SHA written before the spec itself was committed; Story 1.2's review flagged exactly this staleness)
...
- [ ] **Task 4: Update the tests that pin the old front door** (AC: 1, 5, 6 — deliberate revisions, not regressions)
  ...
  - [ ] `src/components/PublicLanding.test.jsx` (new, mirror the established RTL style — DI callbacks, label/role queries, `waitFor`/`findBy*` to settled states):
  ...
  - [ ] Full suite green: all 49 existing tests (minus the two deliberate revisions above) + new ones; `npm run lint`, `npm run typecheck`, `npm run build` all pass.
- [ ] **Task 5: Playwright E2E foothold** (Epic 1 retro action item #1 — lands with this story by design)
  - [ ] `npm i -D @playwright/test` (latest 1.61.x, verified current 2026-07-16) and `npx playwright install chromium` — chromium only for the foothold; more browsers are a later decision, not this story's.
  - [ ] `playwright.config.js` at repo root: `testDir: './e2e'`, one chromium project, `use: { baseURL: 'http://localhost:5173' }`, `webServer: { command: 'npm run dev', url: 'http://localhost:5173', reuseExistingServer: true }`.
  - [ ] ⚠️ **Vitest collection trap:** Vitest's default include pattern will pick up `e2e/*.spec.js` and fail on Playwright imports. Add to `vite.config.js`'s `test` block: `exclude: [...configDefaults.exclude, 'e2e/**']` (import `configDefaults` from `vitest/config`). Verify `npm test` still runs exactly the jsdom suites and nothing under `e2e/`.
  - [ ] New script in `package.json`: `"test:e2e": "playwright test"`. Do NOT fold it into `npm test` — the dev server needs `amplify_outputs.json` (gitignored, sandbox-generated), so e2e runs are a local/sandbox-machine concern for now, not part of the always-on gate.
  - [ ] `e2e/public-landing.spec.js` — one smoke spec, entirely unauthenticated (no Cognito interaction; `getCurrentUser` rejects with no stored session and `App` lands on the public branch):
  ...
  - [ ] This makes live UI verification an agent-runnable script — the whole point of the retro action item. Run it and make it pass before Task 6's manual pass.
- [ ] **Task 6: Live sandbox verification** (AC: all)
  - [ ] No backend deploy (nothing under `amplify/` changes) — `npm run dev` against the existing `tonyreynolds` sandbox, incognito window (no stored session).
  - [ ] Root URL → landing with pitch, no account UI (AC 1). Quick Draw: pick each of a couple of Spreads, draw, Draw Again, Back, load a draw code (AC 2–4).
  ...
  - [ ] Narrow-viewport eyeball: hero, pills row wrap, auth-entry header wrap, embedded selector grid — no clipping or horizontal scroll.
- [ ] **Task 7: Close out (Definition of Done — Epic 1 retro action item #4)**
  ...
  - [ ] Sweep the diff and this story file for live credentials/keys/secrets before committing (no invite-key codes, no AWS identifiers beyond what's already public convention).
